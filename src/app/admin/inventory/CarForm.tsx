...
import { Loader2, Search, Plus, X, Upload } from "lucide-react";
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 max-w-4xl">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
        ...
        <p className="text-gray-500 text-xs mb-4">
          Paste image URLs (from Google Drive, Imgur, your hosting, etc.). First image is the main photo.
        </p>
        <div className="flex gap-2 mb-3">
          <input
            value={newImageUrl}
        ...
        {form.images.length > 0 && (
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
            {form.images.map((url: string, i: number) => (
              <div key={url} className="relative group aspect-[4/3] bg-gray-800 rounded-xl overflow-hidden border border-gray-700">
                <img src={url} alt="" className="w-full h-full object-cover" />
                {i === 0 && (
                  <span className="absolute top-1 left-1 bg-green-500 text-white text-xs px-1.5 py-0.5 rounded font-bold">
                    MAIN
                  </span>
                )}
                <button
